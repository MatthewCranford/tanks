...
def tank(x,y,tur_pos):
	...
	pygame.draw.rect(game_display, black, 
						(x-tank_height, y, tank_width, tank_height))
	pygame.draw.line(game_display, black, (x,y), 
						possible_turrets[tur_pos],turret_width)
...
def explosion(x, y, size=50):
	...
	while explode:
		...
		while magnitude < size:
			...
			pygame.draw.circle(game_display, color_choices[random.randrange(0,4)], 
...
def fire_shell(xy, tank_x, tank_y, tur_pos, gun_power, x_location, barrier_width, 
	random_height, enemy_tank_x, enemy_tank_y):
	...
	while fire:
		...
		pygame.draw.circle(game_display, red,
							(starting_shell[0], starting_shell[1]), 5)
		
		starting_shell[0] -= (12 - tur_pos)*2

		# y = x**2
		starting_shell[1] += int((((starting_shell[0] - xy[0]) 
			* 0.015/(gun_power/50))**2) - (tur_pos+tur_pos/(12-tur_pos)))

		if starting_shell[1] > (display_height - ground_height):
			print("Last shell: ", starting_shell[0], starting_shell[1])
			hit_x = int((starting_shell[0]*display_height-ground_height)
				/starting_shell[1])
...
def e_fire_shell(xy, tank_x, tank_y, tur_pos, gun_power, x_location, barrier_width, 
	random_height, p_tank_x, p_tank_y):
	...
	while not power_found:
		...
		while fire:
			...
			starting_shell[0] += (12 - tur_pos)*2
			starting_shell[1] += int((((starting_shell[0] - xy[0]) 
				* 0.015/(current_power/50))**2) - (tur_pos+tur_pos/(12-tur_pos)))
	...
	while fire:
		...
		gun_power = random.randrange(int(current_power*0.9), 
			int(current_power*1.1))
		starting_shell[1] += int((((starting_shell[0] - xy[0]) 
			* 0.015/(gun_power/50))**2) - (tur_pos+tur_pos/(12-tur_pos)))
...
def game_loop():
	...
	x_location = (display_width/2) + random.randint(-0.1*display_width,
													0.1*display_width)
	random_height = random.randrange(display_height*.1,display_height*0.6)
		...
		for event in pygame.event.get():
			...
			if event.type == pygame.KEYDOWN:
				if event.key == pygame.K_LEFT:
					...
					damage = fire_shell(gun, main_tank_x, main_tank_y, current_tur_pos,
						fire_power, x_location, barrier_width, random_height, 
					...
					for x in range(random.randrange(0,10)):
						...
						if display_width * 0.3 > enemy_tank_x > display_width * 0.03:
							...
							game_display.fill(green, rect=[0, display_height-ground_height, 
					...
					damage = e_fire_shell(enemy_gun, enemy_tank_x, enemy_tank_y, 8,
						50, x_location, barrier_width, random_height, 
						main_tank_x, main_tank_y)
				...
				if event.key == pygame.K_LEFT and tank_move == -5 or \
							event.key == pygame.K_RIGHT and tank_move == 5:
					tank_move = 0
				if event.key == pygame.K_UP and change_tur == 1 or \
							event.key == pygame.K_DOWN and change_tur == -1:
					change_tur = 0
				if event.key == pygame.K_a and power_change == -1 or \
							event.key == pygame.K_d and power_change == 1:
					power_change = 0
